...
public class DersOgrenciController {
	...
	{
		// localhost:8080/dersOgrenci/searchByName
	...
	{
		// localhost:8080/dersOgrenci/searchByName?name=man
		return service.findNameLike(exp);
	}
	*/
	...
	public ResponseEntity<DersOgrenci> getByIdEntity(@PathVariable Long id)
	{
		...
		DersOgrenci dersOgrenci = service.getByID(id);
		if (dersOgrenci != null)
		{
	...
	*/
	...
	public ResponseEntity<String> save(@RequestBody DersOgrenci dersOgrenci, Locale locale)
	{
		// locale değişkeni otomatik olarak accept-language header 'ından inject edildi
		// localhost:8080/dersOgrenci/save
		dersOgrenci = service.save(dersOgrenci);
		//Locale locale = Locale.forLanguageTag("tr");
		String mesaj = messageSource.getMessage("ogretmen.save.succes", null, locale);
		return ResponseEntity.status(HttpStatus.CREATED).body(dersOgrenci.toString() + " " + mesaj + " http://localhost:8080/ogretmen/getById/" + dersOgrenci.getID());
	}
}
